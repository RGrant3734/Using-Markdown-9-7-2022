...
```{r}
my_name <- 'Robert'
name_length <- str_length(my_name)
```
```{r}
paste("My name is", my_name)
paste(my_name, "has", name_length, "letters in it")
```

```{r}
if (exp(pi) == pi^exp(1)){
  paste("They are the same")
  } else if (exp(pi) > pi^exp(1)){
    paste("e to the power pi is greater")
  } else {
    paste("pi to the power e is greater")
}
```
...
```{r}
feet <- readline(prompt = "Enter the ammount of feet:")
inches <- readline(prompt = "Enter the ammount of inches:")
